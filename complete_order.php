...
function createinvoice($con)
{
	...
	$qry = "SELECT  sum(`credit`)-sum(`debit`) as 'balance' FROM `payment`  group by `user_id` having `user_id`=$custid";
	$res = mysqli_query($con,$qry);
	$row=mysqli_fetch_array($res);
	$currnet_balance=$row['balance'];
	
	if($currnet_balance>=$net_total)
		{
		$sql="insert into invoice(cusomer_id,customer_name,inv_date,status,amount,discount,totalamount)
		values('$custid','$custname','$invdate','created','$gross_total','$discount','$net_total')";
		...
		mysqli_query($con,$sql_detail);
			...
			$net_balance=$currnet_balance-$net_total;
			
			$sql_trans="insert into payment(user_id,debit,balance,cdate,comments,invoice_id)
			values('$custid','$net_total','$net_balance','$invdate','$itemname','$invoiceno')"	;
			mysqli_query($con,$sql_trans);			 		

			return "success";
			
			
		}// acc balance check ture part
